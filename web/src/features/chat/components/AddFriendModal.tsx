    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-4">
          ...
          {successMsg && (
            <div className="mb-4 p-3 bg-green-500/10 border border-green-500/30 rounded-xl flex items-center gap-3 animate-fade-in">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 mb-6">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-lg pl-10 pr-4 py-2 text-white focus:outline-none focus:border-indigo-500 transition-all disabled:opacity-50"
                value={searchTerm}
            ...
            <button 
              type="submit"
              disabled={isSearching || !!successMsg}
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto pr-1 custom-scrollbar">
            {results.length === 0 && !isSearching && searchTerm && !successMsg && (
            ...
            {results.map((user) => (
              <div key={user.id} className="flex items-center justify-between p-3 bg-slate-800/40 rounded-xl border border-slate-800">
                ...
                <button
                  ...
                  disabled={isAdding === user.id || !!successMsg} 
                ...
                >
                  {isAdding === user.id ? (
                    ...
                    <>
                      {successMsg && isAdding === user.id ? <CheckCircle2 size={14} /> : <UserPlus size={14} />}
                      {successMsg && isAdding === user.id ? 'Enviado' : 'Añadir'}
                    </>
                  )}
                </button>
              </div>
